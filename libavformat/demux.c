...
#define DURATION_DEFAULT_MAX_RETRY 7
...
static void estimate_timings_from_pts2(AVFormatContext *ic, int64_t old_offset)
{
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        pos = 0;
        ts = ffifmt(ic->iformat)->read_timestamp(ic, i, &pos, DURATION_DEFAULT_MAX_READ_SIZE);
        ...
        do {
            pos -= step;
            ts = ffifmt(ic->iformat)->read_timestamp(ic, i, &pos, pos + step);
            step += step;
        } while (ts == AV_NOPTS_VALUE && pos >= step && step < DURATION_DEFAULT_MAX_READ_SIZE);
...
static void estimate_timings(AVFormatContext *ic, int64_t old_offset)
{
    ...
    } else if (ffifmt(ic->iformat)->read_timestamp &&
