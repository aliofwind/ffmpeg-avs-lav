...
static void set_frame_props(AVSContext *h, AVFrame *frame)
{
    ...
    frame->flags = h->b_interlaced ? (AV_FRAME_FLAG_INTERLACED | AV_FRAME_FLAG_TOP_FIELD_FIRST) : 0;
...
static int cavs_decode_seq_header(AVSContext *h)
{
	...
	h->cur->flags = h->b_interlaced ? (AV_FRAME_FLAG_INTERLACED | AV_FRAME_FLAG_TOP_FIELD_FIRST) : 0;
...
static int cavs_decoder_param_init( AVSContext *h )
{
    ...
    param->pts_in = 0;
    param->dts_in = 0;
    param->user_data_in = 0;
    param->user_data = 0;
    param->output_top_field_first = 0;
    param->output_interlaced = 0;
    param->output_field_info_valid = 0;
